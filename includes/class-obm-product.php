...
if ( ! class_exists( 'WC_Product_Book' ) ) {
    class WC_Product_Book extends WC_Product_Variable {
        public function get_type() { return 'book'; }
    }
}
...
class OBM_Product {
    ...
    public static function init() {
        // Registration
        add_filter( 'product_type_selector', [ __CLASS__, 'add_book_type' ] );
        add_filter( 'woocommerce_product_class', [ __CLASS__, 'set_book_class' ], 10, 2 );
        add_filter( 'woocommerce_data_stores', [ __CLASS__, 'register_data_store' ] );
        add_action( 'init', [ __CLASS__, 'register_book_taxonomy_term' ] );
        
        // Admin UI
        add_action( 'admin_init', [ __CLASS__, 'setup_book_attributes' ] );
        add_action( 'post_submitbox_misc_actions', [ __CLASS__, 'add_sidebar_button' ] );
        add_action( 'admin_footer', [ __CLASS__, 'lock_sku_js' ] );
        add_filter( 'woocommerce_product_data_tabs', [ __CLASS__, 'add_metadata_tab' ] );
        add_action( 'woocommerce_product_data_panels', [ __CLASS__, 'render_metadata_panel' ] );
        add_action( 'woocommerce_process_product_meta', [ __CLASS__, 'save_book_meta' ] );
        add_action( 'woocommerce_variation_options_dimensions', [ __CLASS__, 'add_variant_export_button' ], 10, 3 );
        add_action( 'admin_notices', [ __CLASS__, 'display_export_notices' ] );
        
        // Frontend & Script Injections
        add_action( 'admin_footer', [ __CLASS__, 'inject_admin_js' ] );
        add_action( 'wp_head', [ __CLASS__, 'inject_frontend_styles' ] );
        add_action( 'wp_footer', [ __CLASS__, 'inject_frontend_js' ] );

        // Frontend Logic
        add_filter( 'woocommerce_is_variable_product', [ __CLASS__, 'mark_as_variable' ], 10, 2 );
        add_action( 'woocommerce_book_add_to_cart', 'woocommerce_variable_add_to_cart', 30 );
        add_filter( 'woocommerce_display_product_attributes', [ __CLASS__, 'display_book_metadata_frontend' ], 10, 2 );
    }
    ...
    public static function display_book_metadata_frontend( $product_attributes, $product ) {
        ...
        if ( $parent_product && $parent_product->is_type( 'book' ) ) {
            ...
            $meta_to_display = [
                'isbn'                   => [ 'label' => __( 'ISBN', 'onix-book-manager' ), 'value' => $isbn_value ],
                '_book_publisher_name'   => [ 'label' => __( 'Publisher', 'onix-book-manager' ), 'value' => $publisher ],
                '_book_publication_date' => [ 'label' => __( 'Publication Date', 'onix-book-manager' ), 'value' => $formatted_pubdate ],
                '_book_page_count'       => [ 'label' => __( 'Page Count', 'onix-book-manager' ), 'value' => get_post_meta( $parent_id, '_book_page_count', true ) ],
                '_book_thema_code'       => [ 'label' => __( 'Thema Category', 'onix-book-manager' ), 'value' => $themasubject ],
            ];

            foreach ( $meta_to_display as $key => $data ) {
                if ( ! empty( $data['value'] ) ) {
                    $product_attributes[ 'obm_' . $key ] = [
                        'label' => $data['label'],
    ...
    public static function add_variant_export_button( $loop, $variation_data, $variation ) {
        ...
        $url = wp_nonce_url( add_query_arg( [ 'obm_generate_xml' => 1, 'product_id' => $variation->ID ], admin_url( 'post.php' ) ), 'obm_export_action', 'obm_export_nonce' );
    ...
    public static function display_export_notices() {
        if ( isset( $_GET['obm_error'] ) && $_GET['obm_error'] === 'no_isbn' ) {
    ...
    public static function setup_book_attributes() {
        ...
        $formatname = 'Format'; $formatslug = sanitize_title($formatname);$forfattername = 'Forfatter'; $forfatterslug = sanitize_title($forfattername);
        if ( ! taxonomy_exists( 'pa_' . $formatslug ) ) {
            wc_create_attribute([ 'name' => $formatname, 'slug' => $formatslug, 'type' => 'select', 'order_by' => 'menu_order', 'has_archives' => false ]);
            register_taxonomy( 'pa_' . $formatslug, [ 'product' ] );
        }
        if ( ! taxonomy_exists( 'pa_' . $forfatterslug ) ) {
            wc_create_attribute([ 'name' => $forfattername, 'slug' => $forfatterslug, 'type' => 'select', 'order_by' => 'menu_order', 'has_archives' => true ]);
            register_taxonomy( 'pa_' . $forfatterslug, [ 'product' ] );
        }
        $terms = [__( 'Hardback', 'onix-book-manager' ), __( 'Paperback', 'onix-book-manager' ), __( 'E-book', 'onix-book-manager' )];
        foreach ( $terms as $term ) { if ( ! term_exists( $term, 'pa_' . $formatslug ) ) wp_insert_term( $term, 'pa_' . $formatslug ); }
    }

    public static function add_book_type( $types ) { $types['book'] = __( 'Book (Multi-Format)', 'onix-book-manager' ); return $types; }
    public static function set_book_class( $classname, $product_type ) { return ( $product_type === 'book' ) ? 'WC_Product_Book' : $classname; }
    public static function register_data_store( $stores ) { $stores['product-book'] = 'WC_Product_Variable_Data_Store_CPT'; return $stores; }
    public static function register_book_taxonomy_term() { if ( ! term_exists( 'book', 'product_type' ) ) wp_insert_term( 'book', 'product_type' ); }
    ...
    public static function add_sidebar_button() {
        ...
        if ( $post && $post->post_type === 'product' ) {
            ...
            if ( $product && $product->is_type('book') ) {
                ...
                $url = wp_nonce_url( add_query_arg( [ 'obm_generate_xml' => 1, 'product_id' => $post->ID ], admin_url( 'post.php' ) ), 'obm_export_action', 'obm_export_nonce' );
    ...
    public static function add_metadata_tab( $tabs ) {
        $tabs['obm_metadata'] = [ 'label' => __( 'Book Metadata', 'onix-book-manager' ), 'target' => 'obm_metadata_panel', 'class' => [ 'show_if_book' ], 'priority' => 11 ];
        return $tabs;
    }
    ...
    public static function render_metadata_panel() {
        ...
        <div id="obm_metadata_panel" class="panel woocommerce_options_panel show_if_book">
            <div class="options_group">
                ...
                <?php
                woocommerce_wp_text_input( [ 'id' => '_isbn_value', 'label' => __( 'ISBN-13', 'onix-book-manager' ) ] );
                woocommerce_wp_text_input( [ 'id' => '_book_publisher_name', 'label' => __( 'Publisher', 'onix-book-manager' ), 'placeholder' => get_bloginfo('name') ] );
                ...
                woocommerce_wp_text_input( [ 'id' => '_book_page_count', 'label' => __( 'Number of Pages', 'onix-book-manager' ), 'type' => 'number' ] );
                ?>
    ...
    public static function save_book_meta( $post_id ) {
        $fields = ['_isbn_value', '_book_publisher_name', '_book_thema_code', '_book_publication_date', '_book_page_count'];
